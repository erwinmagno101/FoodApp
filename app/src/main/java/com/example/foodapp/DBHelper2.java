...
public class DBHelper2 extends SQLiteOpenHelper{
    ...
    public boolean deleteReservations(String resid){
        SQLiteDatabase DB = this.getWritableDatabase();
        Cursor cursor = DB.rawQuery("Select * from reservations where res =?", new String[]{resid});
        if(cursor.getCount() > 0){
            long result = DB.delete("reservations", "res =?", new String[]{resid});
            if (result == -1){
                return false;
            }else{
                return true;
