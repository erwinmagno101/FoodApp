    ...
    public void onClick(View view) {
        if (view == btnResConfirm){
            ...
            if (TextUtils.isEmpty(txtDate) || TextUtils.isEmpty(txtTime) || TextUtils.isEmpty(txtGuest) || TextUtils.isEmpty(txtTable) || TextUtils.isEmpty(txtSpecial)){
                ...
                if(Integer.parseInt(txtTable) > 5){
                    Toast.makeText(getContext(), "We only reserve maximum of 5 tables", Toast.LENGTH_SHORT).show();
                    return;
                }
                String sDate1= resDate.getText().toString() + " " + resTime.getText().toString();
                SimpleDateFormat formatter = new SimpleDateFormat("MM/dd/yyyy HH:mm");
                Date date = new Date();
                try {
                    Date date1=new SimpleDateFormat("MM/dd/yyyy HH:mm").parse(sDate1);
                    if (date.before(date1)){
                        ...
                        Toast.makeText(getContext(), "Too Late to reserve", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
